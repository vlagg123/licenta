...
void routing_update_neighbour(uint8_t nodeId, const uint8_t* mac, int8_t rssi) {
    ...
    if (neighbour_count < MAX_NODES) {
        neighbours[neighbour_count] = {nodeId, {0}, rssi, (uint32_t)millis(), true};
...
// formula cost: alpha*hop + beta*rssi_pen
// mod normal: alpha=0.40 beta=0.60 ; mod alerta (latency-aware): alpha=0.60 beta=0.40
float routing_link_cost(int8_t rssi, bool alertMode) {
    const float alpha = alertMode ? 0.60f : 0.40f;
    const float beta  = alertMode ? 0.40f : 0.60f;

    // rssi ideal -40 dBm, range normalizare 60 dBm
    float rssi_pen = constrain((float)(-40 - rssi) / 60.0f, 0.0f, 1.0f);

    return alpha * 1.0f + beta * rssi_pen;
}
...
RouteEntry routing_best_next_hop(uint8_t destination, bool alertMode) {
    ...
    for (int i = 0; i < neighbour_count; i++) {
        ...
        if (neighbours[i].nodeId == destination) {
            float c = routing_link_cost(neighbours[i].rssi, alertMode);
            if (c < best.cost) {
    ...
    for (int i = 0; i < neighbour_count; i++) {
        if (!neighbours[i].alive) continue;
        float c = routing_link_cost(neighbours[i].rssi, alertMode) + 0.5f;
        if (c < best.cost) {
            best.destination = destination;
